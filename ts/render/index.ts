...
export function draw3dModel(context: CanvasRenderingContext2D, width: number, height: number) {
  ...
  for (let i = 0; i < model.Faces.length; i++) {
    ...
    let word = []
    for (let j = 0; j < 3; j++) {
      let x = (parseFloat(vertices[j].x) + 1) * width / 2
      let y = (parseFloat(vertices[j].y) + 1) * height / 2
    ...
    let intensity = Vec3.dot(n, light_dir)
    if (intensity > 0) {
      triangle(_v[0], _v[1], _v[2], image1, [parseInt(intensity * 255 + ''), parseInt(intensity * 255 + ''), parseInt(intensity * 255 + ''), 255])
    }
  }
  image1.flipVertically()
  let imageData = image1.toImageData()
